...
import { loadUser, nextPage, prevPage, switchPage, searchUsers } from '../actions';
...
class Pagination extends Component {
    handleClick = (event) => {
        ...
        if (event.target.name === "previousPage") {
            ...
            if (isSearchModeOn) {
                this.props.searchUsers(searchName, searchPhone, offset)
            } else {
                this.props.loadUser(offset)
            }
...
const mapDispatchToProps = (dispatch) => ({
    loadUser: (offset) => dispatch(loadUser(offset)),
    ...
    searchUsers: (name, phone, offset) => dispatch(searchUsers(name, phone, offset))
})
